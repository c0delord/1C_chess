...
Процедура СоздатьКлетки() Экспорт
	...
	Для Вертикаль = 1 По 8 Цикл
		Для Горизонталь = 1 По 8 Цикл
			...
			НоваяКлетка.Цвет = ?((Вертикаль%2) = (Горизонталь%2), ЦветЧерный, ЦветБелый); 
			Если Горизонталь = 1 Тогда
				НоваяКлетка.ЦветПовышения = ЦветЧерный;
			ИначеЕсли Горизонталь = 8 Тогда
				НоваяКлетка.ЦветПовышения = ЦветБелый;				
			КонецЕсли;
