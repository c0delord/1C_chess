...
Функция ЭтоДопустимоеПеремещение(_фигура, _источник, _назначение, _первый_ход = Истина, _это_взятие = Ложь, _тип_замены = Неопределено) Экспорт
	...
	ИначеЕсли типФигуры = Перечисления.ТипыФигур.Пешка Тогда
		...
		Если (векторГоризонталь = 1*К И векторВертикаль = 0 И НЕ _это_взятие)
			Или (векторГоризонталь = 2*К И векторВертикаль = 0 И _первый_ход)
...
Функция ПроверкаДопустимостиРокировки(_фигура, _назначение, _таб_партия)
...
Функция ОпределитьТипХода(_фигура, _источник, _назначение, _таб_партия, _состояния_фигур, _цвет_игрока) Экспорт
	Если Не ЗначениеЗаполнено(_фигура) Тогда
		Возврат Перечисления.ТипыКоманд.ВыборКлетки;
	КонецЕсли;
	...
	Если ЭтоДопустимоеПеремещение(_фигура, _источник, _назначение, этоПервыйХода, признакВзятия, типЗаменыФигуры) Тогда
		...
		Если признакВзятия Тогда
			Если фигураНазначение.Цвет <> _фигура.Цвет Тогда
				Возврат Перечисления.ТипыКоманд.ВзятиеФигуры;
			Иначе
				Возврат Перечисления.ТипыКоманд.ВыборКлетки;
			КонецЕсли;
		КонецЕсли;
		
		Возврат Перечисления.ТипыКоманд.ПеремещениеФигуры;
	КонецЕсли;
	Возврат Перечисления.ТипыКоманд.ВыборКлетки;
КонецФункции
